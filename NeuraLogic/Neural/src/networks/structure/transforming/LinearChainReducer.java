...
public class LinearChainReducer implements NetworkReducing {
    ...
    @Override
    public NeuralNetwork reduce(DetailedNetwork<State.Neural.Structure> inet, AtomNeuron<State.Neural> outputNeuron) {
        List<Neuron> middleInputNeurons = new ArrayList<>();
        Pair<Iterator<AggregationNeuron>, Iterator<Weight>> inputs = inet.getInputs(outputNeuron);
        inputs.r.forEachRemaining(middleInputNeurons::add);
        ...
        for (int i = sizeBefore - 1; i >= 0; i--) {
            BaseNeuron<Neuron, State.Neural> neuron = inet.allNeuronsTopologic.get(i);
    ...
    private void prune(DetailedNetwork<State.Neural.Structure> inet, BaseNeuron<Neuron, State.Neural> middle) {
        List<Neuron> middleInputNeurons = new ArrayList<>();
        Iterator<Neuron> inputs = inet.getInputs(middle);
        inputs.forEachRemaining(middleInputNeurons::add);
        if (middleInputNeurons.size() == 1) {  //detected linear chain parent -> middle -> child
            BaseNeuron<Neuron, State.Neural> child = (BaseNeuron<Neuron, State.Neural>) middleInputNeurons.get(0);
            Iterator<Neuron> parents = inet.getOutputs(middle);
            if (parents == null) {
                LOG.warning("Neuron has only 1 input but has no output, thus not pruning it.");
                return;
            }
            parents.forEachRemaining(parent -> {
                inet.replaceInput((BaseNeuron<Neuron, State.Neural>) parent, middle, child);
                inet.replaceOutput(child, middle, parent);
            });
        }
    }

    private void prune(DetailedNetwork<State.Neural.Structure> inet, WeightedNeuron<Neuron, State.Neural> middle) {
