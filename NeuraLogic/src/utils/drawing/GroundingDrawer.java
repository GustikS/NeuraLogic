...
public class GroundingDrawer extends Drawer<GroundingSample> {
    ...
    public void loadGraph(GroundingSample obj) {
        ...
        graphviz.addln(sanitize(obj.query.headAtom.literal.toString()) + "[shape = tripleoctagon]");
    ...
    private String drawEdge(Literal groundHead, GroundRule bodyGrounding, GroundHeadRule groundHeadRule) {
        return sanitize(groundHead.toString()) + " -> " + bodyGrounding.hashCode() + " [fontsize=10, color=blue, fontcolor=green, label = " + sanitize(groundHeadRule.toFullString()) + "]";
    }

    private String draw(GroundHeadRule groundHeadRule) {
        return groundHeadRule.hashCode() + " [shape=rarrow, fontsize=10, color=green, label=" + sanitize(groundHeadRule.toFullString()) + "]";
    }

    private String draw(GroundRule groundRule) {
        return groundRule.hashCode() + " [shape=cds, color=red, label=" + sanitize(groundRule.toString()) + "]";
    }

    public String draw(Literal groundHead) {
        return sanitize(groundHead.toString()) + "[color=blue, fontcolor=blue]";
    }

    private String drawEdge(GroundRule bodyGrounding, Literal literal) {
        return bodyGrounding.hashCode() + " -> " + sanitize(literal.toString())+ "[color=red, fontcolor=red]";
    }
    ...
    public String drawEdge(Literal groundHead, GroundHeadRule groundHeadRule) {
        return sanitize(groundHead.toString()) + " -> " + groundHeadRule.hashCode() + "[color=blue]";
    }
